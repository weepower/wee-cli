...
program
	...
	// .option('-t, --template [repo-url]', 'Create a new project from a repo template')
...
const origWeeJson = fs.readJsonSync(`${repoTmpPath}/wee.json`);
const timer = ora('');
...
function generateProject(weeJson) {
	...
	confirmExistingPaths(existing)
		.then((paths) => {
			...
			if (answers.npm) {
				timer.text = 'Installing npm packages';
				timer.start();
				return npmInstall();
			}
...
function confirmExistingPaths(files) {
	...
	files.forEach((p) => {
		...
		if (stat.isDirectory()) {
			...
			question.choices = ['skip', 'overwrite', 'ask'];
...
`);
	...
	if (npmInstalled) {
		console.log(`Run ${chalk.green('wee run')} to start dev server...`);
	} else {
		console.log(`Run ${chalk.green('npm install')}, then ${chalk.green('wee run')} to start dev server...`);
	}
}